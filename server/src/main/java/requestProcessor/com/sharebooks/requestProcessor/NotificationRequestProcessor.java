...
public class NotificationRequestProcessor extends AbstractRequestProcessor {
	...
	private final NotificationService notificationService = (NotificationService) ServiceSource
			.service(ServiceType.NOTIFICATION.desc());
	// private final EntityFactory<Notification> factory =
	// (EntityFactory<Notification>)
	// FactorySource.getEntityFactory(EntityType.NOTIFICATION.desc());
	...
	// the notifications will be fetched for a particular user using the user uid
	// which will be the recepient uid in notifications
